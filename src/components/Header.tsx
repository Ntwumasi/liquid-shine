...
export default function Header() {
  ...
  return (
    <>
        ...
        <div className="container-custom">
          <nav className="flex items-center justify-between h-16">
            ...
            <div className="hidden lg:flex items-center gap-1">
              {navLinks.map((link) => (
                ...
                >
                  {link.label}
                  {isActive(link.href) && (
                    <span className={`absolute bottom-0 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full ${isScrolled ? 'bg-[#004FBB]' : 'bg-[#FFBA00]'}`} />
                  )}
                </Link>
              ))}
              <Link
            ...
            <button
              ...
              className={`lg:hidden relative w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 z-10 ${
                isScrolled
                  ? 'text-gray-700 hover:bg-gray-100'
                  : 'text-white hover:bg-white/10'
              } ${isMenuOpen ? 'bg-[#004FBB] text-white hover:bg-[#003d91]' : ''}`}
              aria-label="Toggle menu"
              aria-expanded={isMenuOpen}
            >
              <div className="w-5 h-4 relative flex flex-col justify-between">
                <span className={`w-full h-0.5 rounded-full transition-all duration-300 origin-left ${isMenuOpen ? 'bg-white rotate-45 translate-y-0' : isScrolled ? 'bg-gray-700' : 'bg-white'}`} />
                <span className={`w-full h-0.5 rounded-full transition-all duration-300 ${isMenuOpen ? 'opacity-0 scale-0' : isScrolled ? 'bg-gray-700' : 'bg-white'}`} />
                <span className={`w-full h-0.5 rounded-full transition-all duration-300 origin-left ${isMenuOpen ? 'bg-white -rotate-45 translate-y-0' : isScrolled ? 'bg-gray-700' : 'bg-white'}`} />
              </div>
            </button>
          </nav>
      ...
      {/* Mobile Menu Panel */}
      <div
        className={`fixed top-0 right-0 h-full w-[85%] max-w-sm bg-white z-50 lg:hidden transition-transform duration-500 ease-out shadow-2xl ${
        ...
        <div className="flex flex-col h-full">
              ...
              {navLinks.map((link, index) => (
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-4 rounded-xl font-medium transition-all duration-300 ${
                    isActive(link.href)
                      ? 'bg-[#004FBB] text-white'
                      : 'text-gray-700 hover:bg-gray-100'
                  }`}
                  style={{ animationDelay: `${index * 50}ms` }}
                >
                  <span className={`w-2 h-2 rounded-full ${isActive(link.href) ? 'bg-[#FFBA00]' : 'bg-gray-300'}`} />
                  {link.label}
                </Link>
              ))}
            </div>
          ...
          <div className="p-6 border-t border-gray-100 bg-gray-50">
            ...
            <a
              ...
              className="flex items-center justify-center gap-2 py-3 text-[#004FBB] font-semibold hover:text-[#003d91] transition-colors"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-center justify-center gap-4 mt-4 pt-4 border-t border-gray-200">
              <a href="https://facebook.com" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-[#004FBB] hover:text-white transition-all">
              ...
              <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-[#004FBB] hover:text-white transition-all">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
